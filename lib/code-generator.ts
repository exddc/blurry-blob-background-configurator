...
/* JSX Component */
<main className="flex flex-col items-center justify-center min-h-screen relative" style={{ backgroundColor: '${config.backgroundColor}' }}>
    ...
    <div className="flex h-[${config.height}px] blur-[${config.blur}px]">
      ${Array.from({ length: config.numBlobs })
        ...
        style={{
          ...
          animation: 'spin ${config.speed * (i + 1)}s linear infinite'
        }}
      />`,
        )
        .join("\n      ")}
